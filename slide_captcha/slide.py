...
def draw_matched_rectangle(coords, background_bytes: bytes):
    # 解码背景图像
    background = decode_image(background_bytes)

    # 根据提供的坐标在背景图像上画一个矩形
    # 矩形的颜色为红色 (0, 0, 255)，线宽为2
    cv2.rectangle(background, (coords[0], coords[1]), (coords[2], coords[3]), (0, 0, 255), 2)

    # 显示图像
    cv2.imshow('Matched Area', background)
...
if __name__ == '__main__':
    ...
    with open("background.png", "rb") as f:
        background = f.read()
    slide_match(background, target)

    matched_coords = slide_match(background, target)["target"]
    marked_image = draw_matched_rectangle(matched_coords, background)
